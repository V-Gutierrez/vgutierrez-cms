...
class CMSApp {
	...
	updateHeader(section) {
		...
		if (section === "dashboard" || section === "profile") {
			...
			primaryAction.querySelector("span").style.display = "none";
		} else {
			primaryAction.style.display = "flex";
			primaryAction.querySelector("span").textContent =
				`Novo ${section === "blog" ? "Post" : "Item"}`;
		}
	}
	...
	renderDashboardWithDefaults() {
		...
		activityContainer.innerHTML =
			'<p class="no-data">Não foi possível carregar atividades recentes</p>';
	}
	...
	renderGallerySection() {
		...
		grid.innerHTML = this.data.gallery
			.map((item) => this.createGalleryCard(item))
			.join("");
	}
		...
		try {
			switch (type) {
				...
				case "gallery":
					...
					this.data.gallery = this.data.gallery.filter(
						(item) => item.slug !== slug,
					);
					break;
			}
	...
	async initializeContentEditor(initialContent = "") {
		...
		if (container && window.editorManager) {
			const { editor, editorId } =
				await window.editorManager.createContentEditor(
					container,
					initialContent,
					"html",
				);
	...
	setupImageUpload() {
		...
		fileInput.addEventListener("change", async (e) => {
			...
			try {
				...
				statusSpan.innerHTML =
					'<i class="fas fa-check text-green-500"></i> Upload concluído!';
				...
				statusSpan.innerHTML =
					'<i class="fas fa-times text-red-500"></i> Falha no upload';
	...
	async handleProfileImageUpload(file) {
		...
		if (!allowedTypes.includes(file.type)) {
			this.showToast(
				...
				"Apenas JPEG, PNG e WebP são permitidos",
			);
			return;
		}
	...
	async saveProfileData() {
		...
		try {
			...
			this.data.profile.personalInfo = {
				...this.data.profile.personalInfo,
		...
		try {
			...
			switch (type) {
				...
				case "gallery":
					...
					// Update local data
					const galleryIndex = this.data.gallery.findIndex(
						(i) => i.slug === slug,
					);
					if (galleryIndex !== -1) {
	...
	collectFormData() {
		...
		switch (this.currentSection) {
			...
			case "gallery":
				...
				formData.year =
					parseInt(document.getElementById("gallery-year")?.value) ||
		...
		switch (type) {
			case "blog":
				grid.innerHTML = items
					.map((item) => this.createBlogCard(item))
					.join("");
				break;
				grid.innerHTML = items
					.map((item) => this.createProjectCard(item))
					.join("");
				break;
			case "gallery":
				grid.innerHTML = items
	...
	renderLibrarySection() {
		...
		grid.innerHTML = this.data.library
			.map((image) => this.createLibraryImageCard(image))
			.join("");
	}
		...
		saveBtn.onclick = async () => {
			...
			try {
				...
				this.showToast(
					...
					`"${title}" foi adicionado à galeria`,
				);
	...
	async handleBlogImagesUpload(files) {
		...
		this.showToast(
			"success",
			"Upload concluído",
			`${totalFiles} imagem(ns) enviada(s)`,
		);
	...
	updateGitButtons(status) {
		...
		if (status.hasUnpushedCommits) {
			...
			pushBtn.title =
				status.branchStatus === "up-to-date"
